...
#if SENSOR_DEBUG
#define DEBUG_PRINT( arg... )   printk( KERN_INFO "sensors_common:" arg )
#else
...
static int get_sensors_delay( unsigned long arg )
{
    ...
    else
    {
        ...
        DEBUG_PRINT( KERN_INFO "[get_sensors_delay] delay_time.type(%ld) g_DelayTime(%lld)\n", __delay_time.type, g_DelayTime[__delay_time.type] );
    }
...
static int set_sensors_delay( unsigned long arg )
{
    ...
    else
    {
        ...
        DEBUG_PRINT( KERN_INFO "[set_sensors_delay] delay_time.type(%ld) delay_time.time(%lld)\n", __delay_time.type, __delay_time.time );
    }
...
static long
sensors_common_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long ret = -EINVAL;
    void __user *argp = (void __user *)arg;
...
static struct file_operations sensors_common_fops = {
    ...
    .unlocked_ioctl = sensors_common_ioctl,
};
