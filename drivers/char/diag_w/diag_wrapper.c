...
void diag_keypad_cmd_rand_ctl(void *key_in)
{
    ...
    printk(KERN_DEBUG "[diag_wrapper]%s:  (key_code:0x%02X)\n", __func__, key_type->key_code);
    ...
    unsigned int iocmd, unsigned long data)
{
    ...
    case DIAG_WRAPPER_IOCTL_A01:
        ...
        else
        {
            ...
            printk(KERN_DEBUG "[diag_wrapper]%s: wait_event_timeout (key_data:0x%02X)\n", __func__,g_rsp_key_data );
            if(timeout == 0)
            {
                g_rsp_key_data = 0x00;
                key_mask = 0;
    ...
    case DIAG_WRAPPER_IOCTL_C05:
        ret = (int)touch_panel_cmd_callback(TOUCH_PANEL_CMD_TYPE_POWER_ON, 0, diag_touch_panel_cmd_pd_mdcmd);
        if (ret != 0)
            ret = DIAG_WRAPPER_IOCTL_NG;
        break;
    ...
    case DIAG_WRAPPER_IOCTL_C14:
        res_size = ( sizeof(char) * 3 );
        req_size = ( sizeof(char) * 4 );
        ...
        ret = (int)touch_panel_cmd(TOUCH_PANEL_CMD_TYPE_SPI_WRITE, pkt_params);
        break;

	
    case DIAG_WRAPPER_IOCTL_C15:
        req_size = ( sizeof(char) * 4 );
        res_size = ( sizeof(char) * 255 + sizeof(char) );
        pkt_params = (char *)kmalloc(res_size, GFP_KERNEL);
        if( !pkt_params )
            return -ENOMEM;
            
        memset(pkt_params, 0x00, res_size );
        err = copy_from_user( pkt_params, (unsigned char *)data, req_size );
        if (err) {
            kfree(pkt_params);
            return -EINVAL;
        }
        ret = (int)touch_panel_cmd(TOUCH_PANEL_CMD_TYPE_SPI_READ, pkt_params);
        break;
